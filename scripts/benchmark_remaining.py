...
# files
in_file = DATA / "chembl_1M538K_to_EOF.smi.gz"
out_file = DATA / "chembl_failed_2.smi"
num_entries = 404005 
(DATA / ".timestamp_2").write_text(timestamp)


def validate_entry(smi):
    ref = Chem.MolFromSmiles(smi)
    if not ref:
        sys.stderr.write(f"\nERROR reading SMILES entry: {smi}\n")
        return None
...
(DATA / ".failed_count_2").write_text(str(count))
(DATA / ".timing").write_text("NaN")
