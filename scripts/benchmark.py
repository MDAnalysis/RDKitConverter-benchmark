...
def validate_inferring(ref: Chem.Mol):
...
def get_failing_smiles(smi):
    ref = Chem.MolFromSmiles(smi)
    return smi if validate_inferring(ref) is False else None
...
if __name__ == "__main__":
    ...
    with mp.Pool(N_WORKERS) as pool, gzip.open(in_file, "rt") as fi, open(
        ...
        for i, result in enumerate(pool.imap_unordered(get_failing_smiles, fi)):
            if result:
                fo.write(result)
                count += 1
